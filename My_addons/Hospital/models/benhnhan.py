...
class QLBN(models.Model):
    ...
    tgravien = fields.Date('Thời gian ra viện:', required=True)
...
class Family(models.Model):
    ...
    nguoithan_id = fields.Char(string='Mã người thân:', required=True, copy=False, readonly=True,
                              default=lambda seft: _('Mã NT'))
    @api.model
    def create(self, values):
        if values.get('nguoithan_id', ('Mã NT')) == ('Mã NT'):
            values['nguoithan_id'] = self.env['ir.sequence'].next_by_code('family.action') or _('Mã NT')
        res = super(Family, self).create(values)
        return res